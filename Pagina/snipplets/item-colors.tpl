            ...
            {% if variation.name in ['Color', 'Cor'] %}
                {% if variation.options | length > 1 %}
                    <div class="item-colors">
                        <a href="{{ product_url_with_selected_variant }}" class="item-colors-bullet item-colors-bullet-text visible-phone">{{ variation.options | length }} {{ 'colores' | translate }}</a>
                        <div class="hidden-phone">
                            {% for option in variation.options | take(5) if option.custom_data %}
                                <div title="{{ option.name }}" data-option="{{ option.id }}" class="js-color-variant item-colors-bullet"><span class="item-colors-bullet-square" style="background: {{ option.custom_data }}"></span></div>
                            {% endfor %}
                            ...
                            {% if own_color_variants and custom_color_variants %}
                                <a href="{{ product_url_with_selected_variant }}" class="item-colors-bullet" title="{{ 'Ver más colores' | translate }}">
                            ...
                            {% elseif own_color_variants > 5 %}
                                <a href="{{ product_url_with_selected_variant }}" class="item-colors-bullet" title="{{ 'Ver más colores' | translate }}">+{{ own_color_variants - 5 }}</a>
                            {% elseif custom_color_variants %}
                                <a href="{{ product_url_with_selected_variant }}" class="item-colors-bullet item-colors-bullet-text" title="{{ 'Ver más colores' | translate }}">{{ custom_color_variants }} {{ 'colores' | translate }}</a>
                            {% endif %}
                        </div>
                    </div>
                {% endif %}
